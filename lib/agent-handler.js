...
const FALLBACK_AUTH =
  "language layer can't authenticate (token expired on the bot-owner host). Deterministic routes still work: /status, /nick, /help, quote-reply, or /<slug> body. Re-auth needed to restore freeform replies.";
...
export function makeAgentHandler({ brain, brainHandlers = null, send, log = () => {} }) {
  return async ({ text, messageId }) => {
    ...
    try {
      ...
      // An auth failure (401/403) is not transient — "try again" is useless
      // advice. Tell the user the language layer can't authenticate so the
      // real fix (re-auth on the owner host) is obvious. See lib/brain.js.
      const fallback = err.isAuthError ? FALLBACK_AUTH : FALLBACK_ERROR;
      try {
        await send({ text: fallback, replyToMessageId: messageId });
      } catch (sendErr) {
