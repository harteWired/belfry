...
export class AgentRelayGuard {
  ...
  check(fromSlug, toSlug, text) {
    ...
    const key = this._dupKey(fromSlug, toSlug, text);
    const expiry = this.recent.get(key);
    if (expiry !== undefined && expiry > t) {
      return { ok: false, reason: 'duplicate' };
    }
    if (!this._takeToken(fromSlug, t)) {
  ...
  /**
   * Batch variant for send_many (#50): one fan-out of the SAME text to many
   * recipients is ONE deliberate act, so it costs ONE token — not one per
   * recipient (the per-recipient charge is what forced senders to hand-
   * throttle fleet-wide notices). Echo dedup still runs PER RECIPIENT and
   * records each (from→to, text) pair, so a follow-up single send_to of the
   * same text to any of them within the TTL is still collapsed — and vice
   * versa. Loop protection is intact: reflection loops are single-recipient
   * shapes, and a runaway batcher is still bounded to the refill rate.
   *
   * Returns { ok: false, reason: 'rate' } when the source is exhausted, or
   * { ok: true, recipients: [{ to, ok, reason? }] } with per-recipient
   * duplicate verdicts (order matches toSlugs).
   */
  checkBatch(fromSlug, toSlugs, text) {
    const t = this.now();
    if (!this._takeToken(fromSlug, t)) {
      return { ok: false, reason: 'rate' };
    }
    const recipients = toSlugs.map((to) => {
      const key = this._dupKey(fromSlug, to, text);
      const expiry = this.recent.get(key);
      if (expiry !== undefined && expiry > t) {
        return { to, ok: false, reason: 'duplicate' };
      }
      this.recent.set(key, t + this.dupTtlMs);
      return { to, ok: true };
    });
  ...
  _dupKey(fromSlug, toSlug, text) {
    return `${fromSlug}>${toSlug}:${text.length}:${hashText(text)}`;
  }

  /** Refill the source's bucket and consume one token. False = exhausted. */
  _takeToken(fromSlug, t) {
