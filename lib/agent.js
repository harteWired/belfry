...
// slug, or a question/comment about it? Imperative → fast-path; question →
// let the agent classify (it'll likely be ask + paraphrase, which is the
// right behavior for "belfry how are you doing?").
//
// Imperative cues: starts with a verb/action ("run", "restart", "fix"…), no
// question mark, no "how/what/why/when/where/are/is/can/does/did/will/should"
// at the very start.
const QUESTION_RE = /\?$/;
const INTERROGATIVE_HEAD_RE = /^(how|what|why|when|where|are|is|can|could|do|does|did|will|would|should|have|has)\b/i;
const COMMA_HEAD_RE = /^[,:]\s*/; // "belfry, ..." or "belfry: ..." — addressing, not commanding

function looksImperative(body) {
  if (typeof body !== 'string' || body.length === 0) return false;
  if (QUESTION_RE.test(body.trim())) return false;
  // Strip leading punctuation that just indicates direct address.
  const stripped = body.replace(COMMA_HEAD_RE, '');
  if (INTERROGATIVE_HEAD_RE.test(stripped)) return false;
  return true;
}
 ...
 * and comments addressed to a slug ("belfry how are you doing?").
 */
