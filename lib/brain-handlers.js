...
export function makeBrainHandlers({
  ...
  nicknames,
  registry,
  // () → federation daemon object (or null when federation is disabled).
  // Thunked like getWatcher because federation is wired during startup and
  // resolved fresh per call. Powers listPeers()'s mesh-wide liveness view.
  getFederation,
  sendTelegram,
  ...
  return {
    ...
    /**
     * Read: federated mesh liveness. Returns this host's letter + its live
     * local slugs, plus every non-stale peer's letter + slugs (from the gossip
     * PeerRegistry) with per-peer ageMs. The fleet self-healing poller reads
     * this from ONE local authed call to compute mesh-wide liveness without any
     * cross-host transport: liveSlugs = union(self.slugs, peers[ageMs<=ttlMs]);
     * a peer ABSENT or ageMs>ttlMs = that host + its agents are down. When
     * federation is disabled this reports self only (peers:[], ttlMs:null).
     */
    listPeers() {
      const fed = typeof getFederation === 'function' ? getFederation() : null;
      const selfSlugs = typeof registry?.knownSlugs === 'function'
        ? [...registry.knownSlugs()].sort()
        : [];
      const pr = fed?.peerRegistry ?? null;
      if (!pr || typeof pr.snapshot !== 'function') {
        return { self: { letter: fed?.selfLetter ?? null, slugs: selfSlugs }, peers: [], ttlMs: null };
      }
      return {
        self: { letter: fed.selfLetter ?? null, slugs: selfSlugs },
        peers: pr.snapshot(),
        ttlMs: pr.ttlMs ?? null,
      };
    },
