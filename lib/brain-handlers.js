...
export function makeBrainHandlers({
  ...
  return {
    ...
    listSessions() {
      const slugs = watcher && typeof watcher.getActiveSlugsFromCache === 'function'
        ? [...watcher.getActiveSlugsFromCache()]
        : [];
    ...
    getSession({ slug }) {
      ...
      const active = watcher?.getActiveSlugsFromCache?.() ?? new Set();
      if (!active.has(slug)) return { error: `no active session named '${slug}'` };
      try {
        // Re-read fresh from disk via the watcher's lastSeen cache. The
        // watcher's chokidar updates lastSeen on every status JSON write,
        // so this is current within one event loop tick of disk truth.
        const cached = watcher?.lastSeen?.get?.(slug);
