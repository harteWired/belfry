...
4. PACK — the daemon needs to fit a longer reply within Telegram's per-message budget. The prompt begins with "PACK to <N> chars:" and is followed by the original text. Compress to ≤N chars while preserving substantive content: keep specific values (filenames, numbers, errors, decisions), drop boilerplate ("I will", "Let me", filler), use abbreviations and short connectives, and prefer paragraphs over lists if it saves space. Reply as plain text only — no tools, no preamble, no quoting. Critical: the user can request the unpacked original separately, so completeness is less important than fitting the budget.
...
/**
 * Build the prefix for a PACK request. The brain matches on the literal
 * "PACK to <N> chars:" sentinel; keep the format stable so changes to the
 * system prompt don't break recognition.
 */
export function PACK_PROMPT_PREFIX(targetChars) {
  return `PACK to ${targetChars} chars:`;
}
