...
// Mesh visibility modes (#39, Telegram surface only for now). 'summary' is
// reserved by the issue but not implemented — an unknown mode falls back to
// the default so a future config doesn't break an older daemon.
const VALID_MESH_MODES = new Set(['none', 'full']);

/**
 * Parse the optional `mesh` block (#39, scoped): per-surface visibility of
 * agent-to-agent relay traffic. Shape:
 *   "mesh": {
 *     "telegram": "none",                        // default: none | full
 *     "telegramOverrides": { "wintermute": "full" }  // per-slug, matches either endpoint
 *   }
 * Override keys may be bare slugs (match any host) or host-qualified
 * (`w/wintermute`). Absent/malformed block → null (feature off).
 */
function parseMeshConfig(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const telegram = VALID_MESH_MODES.has(raw.telegram) ? raw.telegram : 'none';
  const telegramOverrides = {};
  if (raw.telegramOverrides && typeof raw.telegramOverrides === 'object') {
    for (const [slug, mode] of Object.entries(raw.telegramOverrides)) {
      if (VALID_MESH_MODES.has(mode)) telegramOverrides[slug] = mode;
    }
  }
  return { telegram, telegramOverrides };
}
...
export function loadConfig(configPath = DEFAULT_CONFIG_PATH) {
  ...
  return {
    ...
    // Mesh visibility (#39, scoped): which agent-to-agent traffic surfaces on
    // Telegram. Null (no block) = feature off, identical to the prior behavior.
    mesh: parseMeshConfig(parsed.mesh),
    configPath,
    missing: false,
  };
}
...
/**
 * Telegram visibility mode for one agent-to-agent relay: 'full' → mirror the
 * message to Telegram, 'none' → keep it off the phone. An override on either
 * endpoint wins (source checked first), matched exact-then-bare so a bare
 * `wintermute` key also catches the host-qualified `w/wintermute`.
 */
export function meshTelegramMode(config, fromSlug, toSlug) {
  const mesh = config?.mesh;
  if (!mesh) return 'none';
  const bare = (s) => (typeof s === 'string' && /^[a-z0-9]\//.test(s) ? s.slice(2) : s);
  const overrides = mesh.telegramOverrides ?? {};
  for (const key of [fromSlug, bare(fromSlug), toSlug, bare(toSlug)]) {
    if (typeof key === 'string' && Object.prototype.hasOwnProperty.call(overrides, key)) {
