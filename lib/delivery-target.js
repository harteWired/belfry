  ...
  return {
    ...
    deliver(slug, text, originatingMessageId = null, attachment = null) {
      if (FED_SLUG_RE.test(slug)) {
        ...
        // An attachment crosses the mesh as its Telegram file_id — the local
        // path is meaningless on another host; the receiving daemon
        // re-downloads with its own bot token. Only attachments without a
        // file_id (shouldn't happen for photos/documents) are dropped.
        const fedAttachment = (attachment && typeof attachment.fileId === 'string' && attachment.fileId)
          ? {
          ...
          : null;
        if (attachment && !fedAttachment) log(`deliver: dropping attachment for federated "${slug}" (no file_id to forward)`);
        ...
        if (Number.isInteger(originatingMessageId) && Number.isInteger(chatId)) {
          federation.forwardInbound(slug, text, { chatId, originatingMessageId, attachment: fedAttachment })
            .then((r) => {
              if (!r || !r.forwarded) {
