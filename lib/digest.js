...
export class Digest {
  ...
  constructor({ idleMs, windowMs, flush, now = Date.now }) {
    ...
    this.state = new Map();
    /** in-flight flush promises so shutdown can await them */
  ...
  _flush(slug /* , reason */) {
    ...
    // its own error logging — we just keep errors from leaking out and track
    // the promise so flushAll() can await on shutdown.
    const p = (async () => this.flush(slug, entry.events))()
      .catch(() => {})
  ...
  /**
   * Force-flush every pending slug and resolve when all flush callbacks
   * complete (including any in-flight from prior timer fires). Used on
   * shutdown — the daemon awaits this before exiting so digest-buffered
   * events don't get dropped silently.
   */
  async flushAll() {
