...
export async function wireFederation({
  ...
  recordReplyMap = null,
  // Fleet fan-out requested by an authorized mesh agent (2026-07-08 policy:
  // Wintermute only). The daemon injects its onBroadcast orchestrator here;
  // the allowlist gate (fedConfig.broadcastHosts) is enforced below, BEFORE
  // this is called. Null on a daemon with no broadcast role.
  onFedBroadcast = null,
  // Inbound-attachment downloader: async ({ fileId, kind, name }) → local
  // path. Wired on hosts with bot creds (file downloads are not
  // owner-exclusive); null on cred-less nodes → forwarded attachments are
  // logged and dropped, text still delivers.
  ...
  const { hostLetter: selfLetter, hostName, token, peers, priority: selfPriority = null, broadcastHosts = [] } = fedConfig;
  ...
  const onInbound = async (env) => {
    // Attachment rides the envelope as a Telegram file_id; fetch it locally
    // so the session gets a path on ITS OWN disk. Failure degrades to
    // text-only delivery — never blocks the message.
    let localAttachment = null;
    if (env.attachment?.fileId) {
      if (typeof downloadAttachment === 'function') {
        try {
          const p = await downloadAttachment(env.attachment);
          localAttachment = env.attachment.kind === 'photo'
            ? { imagePath: p }
            : { filePath: p, ...(env.attachment.name ? { name: env.attachment.name } : {}) };
        } catch (err) {
          log(`federation: inbound ${env.attachment.kind} for ${env.to.slug} failed to download (${err.message}) — delivering text only`);
        }
      } else {
        log(`federation: inbound ${env.attachment.kind} for ${env.to.slug} but no downloader wired (no bot creds) — delivering text only`);
      }
    }
    const n = registry.deliver(env.to.slug, env.text, null, localAttachment);
    if (n > 0) {
  ...
  // A mesh agent asks THIS host to fan text out to its local sessions.
  // Authorization is two layers: the shared fed bearer (transport — only
  // daemons hold it) plus the per-host broadcastHosts allowlist (policy —
  // which mesh identities may broadcast at all). Default empty = nobody;
  // the fleet sets ["w"] so Wintermute is the only broadcasting agent.
  const onBroadcastEnv = async (env) => {
    const from = `${env.from.host}/${env.from.slug}`;
    if (!broadcastHosts.includes(env.from.host)) {
      log(`federation: broadcast from ${from} REFUSED (host "${env.from.host}" not in broadcastHosts [${broadcastHosts.join(', ')}])`);
      tap?.('fed-broadcast', { from, refused: true });
      return { accepted: false, reason: `host "${env.from.host}" is not broadcast-authorized on this host` };
    }
    if (typeof onFedBroadcast !== 'function') {
      log(`federation: broadcast from ${from} — no local broadcast role, dropping`);
      return { accepted: false, reason: 'this host has no broadcast handler' };
    }
    const { count, slugs } = await onFedBroadcast({
      text: env.text,
      targetSlugs: env.targetSlugs ?? null,
      excludeSlugs: env.excludeSlugs ?? null,
      from,
    });
    tap?.('fed-broadcast', { from, delivered: count, chars: env.text.length });
    return { accepted: true, delivered: count, slugs };
  };

  const server = new FederationServer({ port, bind, token, log, onAnnounce, onMessage, onReply, onInbound, onReplyMap, onBroadcast: onBroadcastEnv });
  ...
  // `skipGuard` (#50): send_many already charged its batch token at the
  // registry, so per-recipient remote hops must not double-charge.
  const relayRemote = async (fromSlug, target, text, { skipGuard = false } = {}) => {
  ...
  const forwardInbound = async (target, text, { chatId, originatingMessageId, attachment = null } = {}) => {
    const owners = peerRegistry.ownerMap(registry.knownSlugs());
    const r = resolveTarget(target, { selfLetter, owners });
    ...
    log(`federation: forwarded inbound ${selfLetter}→${r.hostLetter}/${r.slug} (${text.length} chars${attachment ? `, +${attachment.kind}` : ''}, delivered ${delivered})`);
