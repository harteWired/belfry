...
export function buildEnvelope({
  ...
  if (kind === 'inbound') {
    ...
    // Empty text is legal when an attachment rides along — a caption-less
    // photo is the single most common phone gesture, and it already delivers
    // locally as empty text + image_path. Text-only inbounds stay non-empty.
    if (typeof text !== 'string' || (text.length === 0 && attachment == null)) {
      throw new Error('envelope(inbound): text must be a non-empty string (unless an attachment is present)');
    }
    ...
    // Optional attachment: crosses the mesh as a Telegram file_id — the
    // receiving host downloads it with its own bot token (file access is not
    // owner-exclusive), since the sender's local path is meaningless here.
    if (attachment != null) {
      if (typeof attachment !== 'object'
        || typeof attachment.fileId !== 'string' || attachment.fileId.length === 0
        || !['photo', 'document'].includes(attachment.kind)) {
        throw new Error('envelope(inbound): attachment must be { fileId: string, kind: photo|document, name? }');
      }
      env.attachment = {
