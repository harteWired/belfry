...
export const HELP_TOPICS = ['all', 'routing', 'nicknames', 'status', 'watch', 'agent', 'broadcast'];
...
• watch — tap-to-toggle menu for which projects ping you (see /help watch)
• watch <slug> / unwatch <slug> / watching — manage pings by text
• /all <message> — broadcast to every session at once (see /help broadcast)
• help [topic] — this text. Topics: routing, nicknames, status, watch, agent, broadcast
...
Persisted at ~/.local/state/belfry/nicknames.json.`,
  ...
  watch: `Watch = which projects ping you proactively (on ready / error). Manage it all from here — changes apply instantly, no restart.

  /watch                      tap-to-toggle menu (👁 = watched)
  /watch <slug>               watch (ready + error)
  /watch <slug> ready,error   watch specific events
  /unwatch <slug>             stop watching
  /watching                   list what you're watching

The menu lists every known project (plus anything you're already watching). Tap a row to flip it. Changes layer over your belfry.jsonc and persist to ~/.claude/belfry-subscriptions.json — your hand-edited config is never rewritten.`,
