...
// A host-qualified federation target (`<letter>/<slug>`, #44). A nickname may
// point at one of these; deliveries to it are forwarded over the mesh rather
// than into a local session, so set() skips the active-local-session check.
export const FED_TARGET_RE = /^[a-z0-9]\/[a-z0-9][a-z0-9._-]*$/i;
// Router command tokens that would shadow a same-named nickname on the prefix
// path, so they can't be set as nicknames. Mirrors RESERVED_PREFIX_TOKENS in
// lib/router.js (`all` added with the #30 broadcast command; `help`/`resume`
// were already router commands but had been missing from this set).
const RESERVED = new Set(['status', 'nick', 'unnick', 'nicks', 'help', 'resume', 'all']);
...
export class NicknameRegistry {
  ...
  set(nickname, slug) {
    ...
    // A federated target (`<letter>/<slug>`) is a session on a peer host, not a
    // local one — the mesh validates reachability at delivery time, so skip the
    // active-local-session check. Bare slugs must still be live locally.
    const federated = FED_TARGET_RE.test(slug);
    const active = this.getActiveSlugs();
    if (!federated && !active.has(slug)) {
