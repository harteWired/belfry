...
export class Poller {
  constructor({
    ...
    this.transcribeFn = transcribeFn; // ({audioPath}) => Promise<string|null>; null disables voice
    this.onVoiceWithoutKey = onVoiceWithoutKey; // ({chatId, messageId}) → reply when transcribeFn is unset // path on disk where downloaded photos / voice land; null disables attachments
    this.log = log;
