...
export class Poller {
  constructor({
    ...
    onWatchRequest = null, // /watch /unwatch /watching command handler (#40)
    onWatchToggle = null,  // inline-keyboard watch-toggle callback handler (#40)
    onFullExpand = null,
    onBroadcast = null,
    resolveNickname = () => null,
    resolveFederated = () => null, // (token) → '<letter>/<slug>' | null — gossip-map resolve for bare remote slugs (#44)
    resolveTopic = () => null,
    hasFullStash = () => false,
    ...
    isPreempted = null, // optional () => bool priority gate (#38); true = a higher-priority host should own the bot, so don't poll
    log = () => {},
    ...
    // The `target` contract is deliver + knownSlugs (hasSlug is consulted when
    // present, else knownSlugs().has). A wrapper that implements only deliver()
    // throws "knownSlugs is not a function" the first time a prefix/nick route
    // is taken — caught here at construction instead of at runtime (#44).
    if (typeof target?.deliver !== 'function' || typeof target?.knownSlugs !== 'function') {
      throw new TypeError('Poller target must implement deliver() and knownSlugs()');
    }
    this.target = target;
    ...
    this.onWatchRequest = onWatchRequest;
    this.onWatchToggle = onWatchToggle;
    this.onFullExpand = onFullExpand;
    this.onBroadcast = onBroadcast;
    this.resolveNickname = resolveNickname;
    this.resolveFederated = resolveFederated;
  ...
  async tick() {
    // Priority gate (#38). Before contending for the bot, defer to a
    // higher-priority host that is alive AND reachable on Telegram. Unlike a 409
    // (a real poll round-trip), this is a purely local decision, so we stand by
    // via owner.preempt() — which does NOT advance reachableAt — and never call
    // getUpdates. The gate is absent on single-daemon / unprioritized deploys, so
    // this branch is inert there. Recovery is automatic: once the higher peer
    // stops being reachable (process gone, or egress dead → reachableAt ages
    // out), isPreempted() flips false and we poll and take over.
    if (this.isPreempted && this.isPreempted()) {
      const { changed, waitMs } = this.owner ? this.owner.preempt() : { changed: false, waitMs: 15_000 };
      if (changed) this.log('telegram owner: a higher-priority host owns the bot — standing by');
      await sleep(waitMs);
      return;
    }
  ...
  /**
   * Best-effort attachment extraction: photos and documents (#41 inbound).
   * Returns null when the message carries neither; otherwise
   *   { kind: 'photo'|'document', fileId, name?, imagePath?, filePath? }
   * The fileId is ALWAYS carried even when the local download fails — a
   * federated target's host re-downloads from Telegram with its own bot
   * token (the local path is meaningless across hosts), so a failed
   * owner-side download must not strip the attachment from the forward.
   */
  async extractAttachment(message) {
    if (!this.attachmentDir) return null;
    // Photo: Telegram returns sizes ascending — largest is last.
    if (Array.isArray(message?.photo) && message.photo.length > 0) {
      const largest = message.photo[message.photo.length - 1];
      if (!largest?.file_id) return null;
      const attachment = { kind: 'photo', fileId: largest.file_id };
      try {
        attachment.imagePath = await downloadFile({
    ...
    // Document: same flow; the local path is appended to the delivered TEXT
    // (registry.deliver owns that) so the session's model can Read the file.
    const doc = message?.document;
    if (doc?.file_id) {
      const name = sanitizeFileName(doc.file_name);
      const attachment = { kind: 'document', fileId: doc.file_id, ...(name ? { name } : {}) };
      try {
        attachment.filePath = await downloadFile({
          botToken: this.botToken,
          fileId: doc.file_id,
          destDir: this.attachmentDir,
          destName: `doc-${Date.now()}-${message.message_id ?? 'x'}${name ? `-${name.replace(/\.[^.]*$/, '')}` : ''}`,
          fetchImpl: this.fetchFn,
        });
      } catch (err) {
        this.log(`document download failed (msg ${message.message_id}): ${err.message}`);
      }
      return attachment;
    }
    return null;
  ...
  async process(update) {
    ...
    if (update?.callback_query) {
      ...
      // Watch-toggle taps (#40): belfry:watch:<slug>. Slugs never contain a
      // colon, so a 3-part split with the rest re-joined is safe. Handled
      // before the approval path (whose token shape differs).
      if (parts[1] === 'watch') {
        if (!this.onWatchToggle) {
          this.log('watch-toggle dropped — no handler wired');
          return;
        }
        const slug = parts.slice(2).join(':');
        Promise.resolve(this.onWatchToggle({
          callbackQueryId: cq.id,
          slug,
          chatId: cq.message.chat.id,
          messageId: cq.message.message_id,
        })).catch((err) => this.log(`watch-toggle handler error: ${err.message}`));
        return;
      }
    ...
    const routed = route({
      ...
      resolveFederated: this.resolveFederated,
    ...
    if (routed.action === 'watch-menu' || routed.action === 'watch-set' || routed.action === 'watch-unset' || routed.action === 'watch-list') {
      if (!this.onWatchRequest) {
        this.log(`${routed.action} dropped — no handler wired`);
        return;
      }
      Promise.resolve(this.onWatchRequest(routed))
        .catch((err) => this.log(`watch handler error: ${err.message}`));
      return;
    }
    ...
    if (routed.action === 'broadcast') {
      ...
      // at this /all message).
      Promise.resolve(this.onBroadcast({ text: routed.text, messageId: routed.messageId, source: 'telegram', quiet: routed.quiet === true }))
    ...
    // Only download the attachment once we know the message is going to be
    // delivered to a session. Reserved-command / unmatched / status paths
    // get dropped without writing to disk.
    const attachment = await this.extractAttachment(update.message);
    let fanout = 0;
    try {
    ...
    this.log(`routed ${routed.slug} → ${fanout} instance(s) (${routed.text.length} chars${attachment ? `, +${attachment.kind}` : ''})`);
...
/**
 * Sanitize a Telegram-supplied document file_name for use in a local path and
 * in delivered text: basename only (no traversal), conservative charset,
 * bounded length. Null in → null out.
 */
