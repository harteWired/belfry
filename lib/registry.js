...
// send_many (#50): bound one batch. The Jinn-crash recovery fanned to ~18
// sessions; 32 leaves headroom without making a batch a broadcast substitute.
const MAX_SEND_MANY_RECIPIENTS = 32;
// Bare slug, or a host-qualified `<letter>/<slug>` target (#29) — shared by
// /send-to and /send-many.
const SEND_TARGET_RE = /^([a-z0-9]\/)?[a-zA-Z0-9._-]{1,128}$/;
...
export class Registry {
  constructor({
    ...
    brainHandlers = null,
    relayGuard = null,
    // Local POST /broadcast policy. Default TRUE here for back-compat
    // (tests/embedders); the DAEMON always passes an explicit value, and its
    // default is false (closed) per the 2026-07-08 wintermute-only policy.
    ...
    this.allowLocalBroadcast = allowLocalBroadcast;
    /**
     * Wintermute message-flow tap (#49): `(kind, fields) => void`, metadata
     * only, fire-and-forget. Late-bound via setTap() so the daemon can wire it
     * after construction. Null (default) = feature off, zero behavior change.
     * @type {null | ((kind: string, fields?: object) => void)}
     */
    this.tap = null;
    /**
    ...
    /**
     * Optional agent-relay observer (#39, scoped): called once per
     * relayAgentMessage delivery attempt (post-guard, including delivered:0
     * drops) with { from, to, text, delivered }. The daemon wires it to the
     * mesh-mirror (surface selected a2a traffic on Telegram). Receiver-side
     * only by construction — federation-outgoing relays route before
     * relayAgentMessage — so a fleet-wide deploy never double-mirrors.
     * Fire-and-forget: an observer failure never affects the relay.
     * @type {null | ((args: { from: string, to: string, text: string, delivered: number }) => void)}
     */
    this.onAgentRelay = null;
    /**
     * Optional same-host Telegram bridge target: { slug, deliver(fromSlug,
     * text) → Promise }. When a relay's toSlug matches, the message posts to
     * the human's Telegram instead of looking for a session — the local twin
     * of federation-daemon's cross-host telegramBridge (#44). Wired by the
     * daemon only when it has bot creds; null everywhere else (the slug then
     * behaves like any other: no session → delivered 0).
     * @type {null | { slug: string, deliver: (fromSlug: string, text: string) => Promise<unknown> }}
     */
  ...
  /**
   * Late-binding setter for the agent-relay observer (#39). The daemon builds
   * the mesh-mirror after the registry exists (it closes over sendOutbound +
   * the loaded config), so it must be attachable post-construction.
   */
  setAgentRelayObserver(fn) {
    this.onAgentRelay = fn;
  }

  /**
   * Late-binding setter for the same-host Telegram bridge target. The daemon
   * wires it after federation config resolves the bridge slug (default
   * "telegram") — and only when it holds bot creds.
   */
  setHumanTarget(target) {
    this.humanTarget = target;
  }

  /** Fire the agent-relay observer without letting it break the relay. */
  _notifyAgentRelay(from, to, text, delivered) {
    if (!this.onAgentRelay) return;
    try {
      this.onAgentRelay({ from, to, text, delivered });
    } catch (err) {
      this.log(`agent-relay observer failed (${from}→${to}): ${err.message}`);
    }
  }
  ...
  /**
   * Late-binding setter for the Wintermute message-flow tap (#49). Wired by
   * bin/belfry.js when WINTERMUTE_TAP_URL is set; null otherwise.
   */
  setTap(fn) {
    this.tap = fn;
  }
  ...
  deliver(slug, text, originatingMessageId = null, attachment = null) {
    ...
    if (!ids || ids.size === 0) {
      ...
      this.tap?.('deliver', { to: slug, delivered: 0, chars: text.length });
      return 0;
    }
    ...
    if (attachment && typeof attachment === 'object') {
      ...
      // Inbound document (#41): channel injection has no document param the
      // harness renders, so surface it as text — the model Reads the file from
      // the local path. Appended HERE because this is the one local-delivery
      // chokepoint (poller-local, fed onInbound, local-self redeliver), and it
      // needs zero spoke/broker changes.
      if (typeof attachment.filePath === 'string') {
        item.text = `${text}\n\n[attached file${attachment.name ? ` "${attachment.name}"` : ''} saved to ${attachment.filePath} — Read it from there]`;
      }
    }
    let notified = 0;
    const now = Date.now();
    ...
    this.tap?.('deliver', { to: slug, delivered: notified, chars: text.length });
    return notified;
  }
  ...
  broadcast(text, { targetSlugs = null, excludeSlugs = null } = {}) {
    ...
    this.tap?.('broadcast', { count, slugs: [...slugs], chars: text.length });
   ...
   */
  /**
   * Count how many instances a broadcast with these filters WOULD reach,
   * without delivering. Same filter logic as broadcast(). Lets the daemon
   * skip the Telegram placeholder/confirmation for an anchorless (CLI)
   * broadcast that has no recipients — a headless script pinging /broadcast
   * for a slug that lives elsewhere shouldn't spam the human's phone.
   */
  matchBroadcast({ targetSlugs = null, excludeSlugs = null } = {}) {
    const target = targetSlugs ? new Set(targetSlugs) : null;
    const exclude = excludeSlugs ? new Set(excludeSlugs) : null;
    let count = 0;
    for (const inst of this.instances.values()) {
  ...
  relayAgentMessage(fromSlug, toSlug, text, { skipGuard = false } = {}) {
    if (this.relayGuard && !skipGuard) {
      const verdict = this.relayGuard.check(fromSlug, toSlug, text);
      if (!verdict.ok) {
        this.log(`registry: relay ${fromSlug}→${toSlug} blocked (${verdict.reason})`);
        this.tap?.('send_to', { from: fromSlug, to: toSlug, delivered: 0, blocked: verdict.reason });
        return { ok: false, delivered: 0, reason: verdict.reason };
      }
    }
    // Telegram bridge target, same-host case. Cross-host sends to the bridge
    // slug are handled by federation-daemon's onMessage (#44); this closes the
    // gap for a LOCAL session (or a single-host deployment) addressing it —
    // without it, a same-host send_to("telegram") fell through to "no live
    // session" and silently dropped. Post-guard on purpose: pushes to the
    // human's phone deserve the flood protection most. Fire-and-forget like
    // the fed path's deliver — the send-queue owns retries; a failure is
    // logged, not surfaced (the message is already off the mesh).
    if (this.humanTarget && toSlug === this.humanTarget.slug) {
      Promise.resolve(this.humanTarget.deliver(fromSlug, text))
        .catch((err) => this.log(`registry: relay ${fromSlug}→${toSlug} telegram-bridge send failed: ${err.message}`));
      this.log(`registry: relay ${fromSlug}→${toSlug} → telegram bridge`);
      this.tap?.('send_to', { from: fromSlug, to: toSlug, delivered: 1, bridged: true, chars: text.length });
      this._notifyAgentRelay(fromSlug, toSlug, text, 1);
    ...
    if (!ids || ids.size === 0) {
      ...
      this.tap?.('send_to', { from: fromSlug, to: toSlug, delivered: 0 });
      this._notifyAgentRelay(fromSlug, toSlug, text, 0);
    ...
    this.tap?.('send_to', { from: fromSlug, to: toSlug, delivered, chars: text.length });
    this._notifyAgentRelay(fromSlug, toSlug, text, delivered);
    return { ok: true, delivered };
  }

  /**
   * send_many (#50): fan ONE text out to many recipients in a single call —
   * the daemon owns the fan-out, so the sender pays one guard token instead
   * of hand-throttling N send_to calls against the per-source bucket (the
   * Jinn-crash recovery friction). Each recipient routes exactly like a
   * /send-to target: federation router first for `<letter>/<slug>` peers,
   * then the local relay — with the guard bypassed per-recipient because the
   * batch was already charged (checkBatch also ran the per-recipient echo
   * dedup, so repeats within the TTL come back as `duplicate` here).
   *
   * Returns { ok: false, reason: 'rate' } when the source bucket is dry, or
   * { ok: true, results: [{ to, ok, delivered, remote?, host?, reason? }] }
   * in input order (after de-duplicating repeated slugs).
   */
  async relayMany(fromSlug, toSlugsRaw, text) {
    const toSlugs = [...new Set(toSlugsRaw)];
    const dupByTarget = new Map();
    if (this.relayGuard) {
      const verdict = this.relayGuard.checkBatch(fromSlug, toSlugs, text);
      if (!verdict.ok) {
        this.log(`registry: send_many ${fromSlug}→${toSlugs.length} recipient(s) blocked (${verdict.reason})`);
        this.tap?.('send_many', { from: fromSlug, count: toSlugs.length, delivered: 0, blocked: verdict.reason });
        return { ok: false, reason: verdict.reason };
      }
      for (const r of verdict.recipients) dupByTarget.set(r.to, r);
    }
    const results = await Promise.all(toSlugs.map(async (target) => {
      const dup = dupByTarget.get(target);
      if (dup && !dup.ok) return { to: target, ok: false, delivered: 0, reason: dup.reason };
      let localSlug = target;
      if (this.federationRouter) {
        let remote;
        try {
          remote = await this.federationRouter(fromSlug, target, text, { skipGuard: true });
        } catch (err) {
          this.log(`registry: federation router threw for ${fromSlug}→${target}: ${err.message}`);
          remote = { handled: true, ok: false, reason: `federation error: ${err.message}` };
        }
        if (remote && remote.handled) {
          return remote.ok
            ? { to: target, ok: true, delivered: remote.delivered ?? 1, remote: true, host: remote.host }
            : { to: target, ok: false, delivered: 0, reason: remote.reason };
        }
        if (remote && remote.localSlug) localSlug = remote.localSlug;
      }
      const r = this.relayAgentMessage(fromSlug, localSlug, text, { skipGuard: true });
      return { to: target, ok: r.ok, delivered: r.delivered, ...(r.reason ? { reason: r.reason } : {}) };
    }));
    const delivered = results.reduce((n, r) => n + (r.delivered || 0), 0);
    this.tap?.('send_many', { from: fromSlug, count: toSlugs.length, delivered, chars: text.length });
  ...
  async handle(req, res) {
    ...
    if (req.method === 'POST' && pathname === '/send-many') return this.handleSendMany(req, res);
  ...
  async handleBrain(req, res, pathname) {
    ...
    const ROUTES = {
      ...
      '/brain/list-peers': { method: 'GET', fn: 'listPeers' },
  ...
  async handleBroadcast(req, res) {
    // Broadcast policy (2026-07-08): the local /broadcast route is CLOSED by
    // default. Any local process holding the loopback registry token could
    // fan text into every session (and generate Telegram traffic) — that
    // primitive is now reserved for authorized mesh agents via /fed/broadcast
    // (allowlisted per host; the fleet allows only Wintermute). The daemon
    // reopens this route only when BELFRY_BROADCAST_LOCAL is set (for the
    // human's own belfry-broadcast CLI on that host).
    if (!this.allowLocalBroadcast) {
      res.writeHead(403, { 'content-type': 'application/json' });
      res.end(JSON.stringify({
        ok: false,
        error: 'local /broadcast is disabled — broadcast is restricted to authorized mesh agents (set BELFRY_BROADCAST_LOCAL=1 on the daemon to re-enable the local CLI)',
      }));
      return;
    }
    const body = await readJson(req);
  ...
  async handleSendTo(req, res) {
    ...
    if (
      ...
      !SEND_TARGET_RE.test(body.to_slug) ||
    ...
    if (this.federationRouter) {
      ...
      // Router resolved a self-qualified target ("j/x" on host j) to local —
      // deliver under the bare slug the local index actually knows.
      if (remote && remote.localSlug) toSlug = remote.localSlug;
    }

    const result = this.relayAgentMessage(inst.slug, toSlug, body.text);
    if (!result.ok) {
  ...
  /**
   * `POST /send-many` — batch agent-to-agent relay (#50). Body:
   *   { instance_id, to_slugs: [...], text }
   * One text fanned to up to MAX_SEND_MANY_RECIPIENTS targets (bare or
   * host-qualified), charged ONE guard token for the whole batch. Returns
   * { ok, results: [{ to, ok, delivered, remote?, host?, reason? }] } — a
   * recipient with no live session is ok:true delivered:0, same contract as
   * /send-to. 429 with { reason: 'rate' } only when the source bucket is dry.
   */
  async handleSendMany(req, res) {
    const body = await readJson(req);
    if (
      !body ||
      typeof body.instance_id !== 'string' ||
      body.instance_id.length === 0 ||
      !Array.isArray(body.to_slugs) ||
      body.to_slugs.length === 0 ||
      body.to_slugs.length > MAX_SEND_MANY_RECIPIENTS ||
      !body.to_slugs.every((s) => typeof s === 'string' && SEND_TARGET_RE.test(s)) ||
      typeof body.text !== 'string' ||
      body.text.length === 0
    ) {
      res.writeHead(400, { 'content-type': 'text/plain' });
      res.end(`bad request (need instance_id, 1-${MAX_SEND_MANY_RECIPIENTS} valid to_slugs, text)`);
      return;
    }
    if (body.text.length > MAX_SEND_TEXT_LEN) {
      res.writeHead(413, { 'content-type': 'text/plain' });
    ...
    inst.lastSeen = Date.now();
    const result = await this.relayMany(inst.slug, body.to_slugs, body.text);
    if (!result.ok) {
      res.writeHead(429, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ ok: false, reason: result.reason }));
      return;
    }
    const delivered = result.results.reduce((n, r) => n + (r.delivered || 0), 0);
    res.writeHead(200, { 'content-type': 'application/json' });
    res.end(JSON.stringify({ ok: true, delivered, results: result.results }));
  }
