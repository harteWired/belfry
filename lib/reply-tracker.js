 ...
 * every record). Disk writes are coalesced via setImmediate so a burst of
 * record() calls in the same tick — e.g. multiple slugs reaching ready at
 * once, the digest flush firing for several slugs in parallel — produces
 * one write per tick instead of N. Load failures and write failures are
 * logged and ignored; the worst case degrades to in-memory-only.
 */
...
export class ReplyTracker {
  constructor({ capacity = DEFAULT_CAPACITY, persistPath = null, log = () => {} } = {}) {
    ...
    /** setImmediate handle when a save is queued for this tick. */
    this.saveTimer = null;
  ...
  /** Force a synchronous save — used on shutdown so pending writes flush. */
  flush() {
    if (this.saveTimer) {
      clearImmediate(this.saveTimer);
      this.saveTimer = null;
      this.#save();
    }
  }
  ...
  #scheduleSave() {
    ...
    if (this.saveTimer) return; // already queued for this tick
