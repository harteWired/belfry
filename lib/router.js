...
// "all good" — to safely treat as a command). An optional `!` (`/all! <body>`)
// makes it FIRE-AND-FORGET: fan the command out but DON'T mark every session as
// owing a reply or seed the completion tracker — so a command-broadcast like
// `/all! compress` doesn't generate N auto-replies that flood the chat into a
// 429. Group 1 = the `!` (quiet flag), group 2 = the body.
...
export function route({
  ...
  if (typeof message.reply_to_message?.message_id !== 'number') {
    ...
    if (allMatch) {
      const quiet = allMatch[1] === '!';
      const body = (allMatch[2] ?? '').trim();
      if (body.length > 0) return { action: 'broadcast', text: body, quiet, messageId };
      // Bare "/all" (or "/all!") with no body → fall through to unmatched.
    }
  }
