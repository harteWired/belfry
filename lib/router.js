/**
 * Route a Telegram update to (slug, text).
 *
 * Steps:
 ...
 *      text as the body (prefix path).
 *   5. Otherwise → null.
 *
 * On a successful route, also returns `messageId` — the originating Telegram
 * message_id of the inbound user message. The registry uses this to mark the
 * slug as "owes a reply" so the auto-reply path can quote-reply on next ready.
 */
...
export function route({ update, expectedChatId, replyTracker, knownSlugs }) {
  ...
  if (m) {
    ...
    if (knownSlugs.has(slug) && body.length > 0) {
      return { slug, text: body, messageId };
    }
  }
