 ...
 * checked. Most updates (wrong chat, no text, /status, quote-reply) return
 * before it's invoked, so callers should pass a cheap accessor (e.g. a Map's
 * `.has` bound) rather than materializing a Set on every route call.
 *
 * Backwards compat: if `knownSlugs` (a Set) is passed instead of `hasSlug`,
 * we use it the same way. New callers should use `hasSlug`.
 */
export function route({ update, expectedChatId, replyTracker, hasSlug, knownSlugs }) {
  ...
  // /<slug-name> prefix path. Only here do we touch the slug set/registry.
  const m = text.match(PREFIX_RE);
  if (m) {
    const slug = m[1];
    const body = (m[2] ?? '').trim();
    if (body.length === 0) return null;
    const known = hasSlug ? hasSlug(slug) : knownSlugs?.has(slug);
    if (known) return { action: 'deliver', slug, text: body, messageId };
  }
