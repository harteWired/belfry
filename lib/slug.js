 ...
 *
 * Mirrors the logic in claudelike-bar's hooks/dashboard-status.js so that
 * the slug a belfry hook computes matches the slug claudelike-bar wrote to
 * /tmp/claude-dashboard/<slug>.json — both must agree, or the inbox key
 * the hook drains will never match the inbox key Telegram replies were
 * pushed under.
 *
 * Order of resolution:
