...
// Status JSONs are tiny by spec (a few KB max). Cap reads so a malformed or
// hostile file at /tmp/claude-dashboard/<slug>.json can't force a giant
// allocation on the daemon. /status is user-trigger-able from Telegram, so
// without this an attacker who can plant files in the dir could amplify
// memory pressure on every /status call.
const MAX_STATUS_BYTES = 256 * 1024;

function readStatusFile(filePath) {
  try {
    const stat = fs.statSync(filePath);
    if (stat.size > MAX_STATUS_BYTES) return null;
