...
export class SubscriptionsStore {
  ...
  /**
   * Every slug the store has an opinion about — watched OR explicitly unwatched.
   * The /watch menu unions this in so a project you toggle OFF stays visible
   * (and re-watchable) even when it has no live spoke and no dashboard file —
   * otherwise unwatching it would make the row vanish with no way back.
   */
  managedSlugs() {
    const set = new Set(Object.keys(this.overrides));
    for (const s of Object.keys(this.subscriptions)) if (this.subscriptions[s]) set.add(s);
    return [...set];
  }
