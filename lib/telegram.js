...
const DEFAULT_FILE_CAP = 4 * 1024 * 1024; // 4 MB; photos from phones are usually well under this
// Telegram's Bot API caps getFile downloads at 20 MB — a higher configured cap
// can't be honoured, so it's the ceiling we clamp BELFRY_ATTACHMENT_MAX_BYTES to.
export const TELEGRAM_DOWNLOAD_CEILING = 20 * 1024 * 1024;

/**
 * Parse a human-friendly byte size — plain bytes (`4194304`) or a unit suffix
 * (`8mb`, `512k`, `1.5m`). Returns a positive integer byte count, or null if
 * the input is empty/unparseable so the caller can fall back to a default.
 */
export function parseByteSize(raw) {
  const m = /^\s*(\d+(?:\.\d+)?)\s*(g|gb|m|mb|k|kb|b)?\s*$/i.exec(raw ?? '');
  if (!m) return null;
  const val = Number(m[1]);
  if (!Number.isFinite(val) || val <= 0) return null;
  const mult = { b: 1, k: 1024, kb: 1024, m: 1024 ** 2, mb: 1024 ** 2, g: 1024 ** 3, gb: 1024 ** 3 };
  return Math.round(val * mult[(m[2] || 'b').toLowerCase()]);
}

/**
 * Resolve the inbound-attachment size cap from `BELFRY_ATTACHMENT_MAX_BYTES`.
 * Unset/invalid → the 4 MB default; a valid value is clamped to Telegram's
 * 20 MB getFile ceiling (a larger request can't actually be downloaded).
 */
export function resolveFileCap(env = process.env) {
  const parsed = parseByteSize(env.BELFRY_ATTACHMENT_MAX_BYTES);
  if (parsed == null) return DEFAULT_FILE_CAP;
  return Math.min(parsed, TELEGRAM_DOWNLOAD_CEILING);
}

// Baked once at module load — the daemon reads env at launch (per-process, not per-call).
const TELEGRAM_FILE_CAP = resolveFileCap();
...
// Extensions Telegram renders inline as a photo (with compression). Everything
// else goes as a document so the original bytes/quality are preserved.
const PHOTO_EXTS = new Set(['.jpg', '.jpeg', '.png', '.gif', '.webp']);
const TELEGRAM_CAPTION_CAP = 1024;

/**
 * Send a local file to a Telegram chat (#outbound files). Picks sendPhoto for
 * image extensions (inline preview) and sendDocument otherwise (original
 * bytes). Multipart upload built on Node 20's global FormData/Blob — no SDK,
 * matching the project's no-dependency rule (mirrors lib/voice.js's inbound
 * upload). `caption` is optional and clamped to Telegram's 1024-char media cap.
 * The caller is responsible for validating the path exists and is within size
 * limits; readFileSync throws otherwise and the caller logs + skips.
 */
export async function sendDocument({ botToken, chatId, filePath, caption, forumTopicId, replyToMessageId, forceDocument = false, fetchImpl = fetch, fs = nodeFs, path = nodePath }) {
  const ext = path.extname(filePath).toLowerCase();
  const asPhoto = !forceDocument && PHOTO_EXTS.has(ext);
  const method = asPhoto ? 'sendPhoto' : 'sendDocument';
  const field = asPhoto ? 'photo' : 'document';
  const bytes = fs.readFileSync(filePath);
  const form = new FormData();
  form.append('chat_id', String(chatId));
  if (caption) form.append('caption', String(caption).slice(0, TELEGRAM_CAPTION_CAP));
  if (forumTopicId) form.append('message_thread_id', String(Number(forumTopicId)));
  if (typeof replyToMessageId === 'number' && replyToMessageId > 0) {
    form.append('reply_parameters', JSON.stringify({ message_id: replyToMessageId, allow_sending_without_reply: true }));
  }
  // Blob over the raw bytes; the filename is what the recipient sees.
  ...
  if (!res.ok) {
    const errBody = await res.text().catch(() => '');
    throw telegramHttpError(method, res.status, errBody);
  }
  const json = await res.json();
  if (!json.ok) {
    throw new Error(`telegram ${method} returned ok=false: ${JSON.stringify(json).slice(0, 200)}`);
  }
  return json.result;
}
 ...
 *
 * Returns the absolute path the bytes landed at. Capped to bound daemon memory
 * and reject pathological payloads — 4 MB by default (covers any phone
 * screenshot), overridable via BELFRY_ATTACHMENT_MAX_BYTES up to Telegram's
 * 20 MB getFile ceiling. An oversized file throws (caller degrades to
 * text-only); the cap is enforced twice — the declared getFile size first
 * (cheap, before streaming) and the streamed byte length as a backstop.
 *
