...
// POSIX; fall back to os.tmpdir() on Windows, where /tmp doesn't exist.
//
// Dir precedence must stay in LOCK-STEP with bin/belfry-hook.js resolveStatusDir
// (Status-File Contract v1 §A): CLAUDELIKE_STATUS_DIR (canonical) →
// CLAUDE_DASHBOARD_DIR (deprecated alias) → the platform default.
const STATUS_DIR = (process.env.CLAUDELIKE_STATUS_DIR || '').trim()
