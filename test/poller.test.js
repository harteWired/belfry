...
test('tick: voice note transcribes via injected transcribeFn and routes the transcript', async () => {
  const updates = [
    {
      update_id: 2100,
      message: {
        message_id: 21,
        chat: { id: CHAT },
        voice: { file_id: 'voice123', duration: 12, mime_type: 'audio/ogg' },
      },
    },
  ];
  const fetchFn = async (url) => {
    if (url.endsWith('/getUpdates')) return { ok: true, json: async () => ({ ok: true, result: updates }) };
    if (url.endsWith('/getFile')) return { ok: true, json: async () => ({ ok: true, result: { file_path: 'voice/v.ogg', file_size: 100 } }) };
  ...
  const target = {
    delivered: [],
    deliver(slug, text) { this.delivered.push({ slug, text }); return 1; },
    knownSlugs() { return new Set(['life-planner']); },
  };
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'belfry-voice-'));
  ...
  const poller = new Poller({
    ...
    transcribeFn: async ({ audioPath }) => {
      transcribeCalls.push(audioPath);
      return '/life-planner do the thing'; // route via prefix
    },
  });
  await poller.tick();
  ...
  assert.equal(target.delivered[0].text, 'do the thing');
  fs.rmSync(tmpDir, { recursive: true, force: true });
});

test('tick: voice without transcribeFn invokes onVoiceWithoutKey, no delivery', async () => {
  const updates = [
    {
      update_id: 2200,
      message: {
  ...
  const poller = new Poller({
    ...
    fetchFn: fakeOk(updates),
    transcribeFn: null,
    onVoiceWithoutKey: async (args) => { seen.push(args); },
  });
  await poller.tick();
  await new Promise((r) => setTimeout(r, 10));
  assert.equal(seen.length, 1);
...
test('tick: voice over 60s cap is dropped (no transcribe attempt)', async () => {
  const updates = [
    {
      update_id: 2300,
      message: {
  ...
  const poller = new Poller({
    ...
    target,
    fetchFn: fakeOk(updates),
    transcribeFn: async () => { transcribeCalls.push(1); return 'should not happen'; },
    attachmentDir: '/tmp',
  });
  await poller.tick();
  await new Promise((r) => setTimeout(r, 10));
