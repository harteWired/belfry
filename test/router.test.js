...
test('quote-reply routes to tracked slug', () => {
  ...
  assert.deepEqual(r, { slug: 'life-planner', queue: 'continuation', text: 'do the thing' });
});
...
test('quote-reply with untracked id falls through to prefix path', () => {
  ...
  assert.deepEqual(r, { slug: 'belfry', queue: 'continuation', text: 'restart' });
});
...
test('prefix path with known slug routes', () => {
  ...
  assert.deepEqual(r, { slug: 'life-planner', queue: 'continuation', text: 'do X' });
});
